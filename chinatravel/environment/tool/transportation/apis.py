...
class Transportation:
    ...
    def goto(self, city, start, end, start_time, transport_type, verbose=False):
        ...
        if transport_type == "walk":
            ...
            if verbose:
                print(
                    "Walk Distance {:.3} kilometers, Time {:.3} hour, Cost {}Â¥".format(
            ...
            if verbose:
                print(
                    "Taxi Distance {:.3} kilometers, Time {:.2} hour, Cost {}Â¥".format(
                        distance, time, int(cost)
                    )
                )
            return transports
            ...
            if stationA and stationB:
                ...
                transports.append(
                    {
                        "start": stationA["name"] + "-å°éç«",
                        "end": stationB["name"] + "-å°éç«",
                        "mode": "metro",
                ...
                if verbose:
                    print(
                        "Walk: From starting point to metro {}, Distance: {}.".format(
                            stationA["name"] + "-å°éç«", distanceA
                        )
                    )
                    print(
                        f"Subway: From {stationA['name'] + '-å°éç«'} to {stationB['name'] + '-å°éç«'}: {' -> '.join(shortest_path)}"
                    )
                    print("The cost of subway: {}Â¥".format(cost))
                    print(
                        "Walk: From metro {} to ending point,  Distance: {}.".format(
                            stationB["name"] + "-å°éç«", distanceB
                        )
                    )
                return transports
            else:
